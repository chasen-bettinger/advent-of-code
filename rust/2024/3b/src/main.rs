...
fn get_valid_multiplications(memory: &str) -> Result<Vec<(i32, i32)>, Box<dyn Error>> {
    let re =
        Regex::new(r"(?<mul>mul)\((?<a>[0-9]{1,3}),(?<b>[0-9]{1,3})\)|(?<op>don't|do)").unwrap();
    let mut is_dont = false;
    let matches: Vec<(i32, i32)> = re
        .captures_iter(memory)
        .filter_map(|x| {
            let operation = x.name("op");
            if operation.is_some() {
                let op = operation.unwrap().as_str();
                is_dont = op == "don't";
                return None;
            }
            if is_dont {
...
mod tests {
    ...
    fn simple_case() {
        ...
        assert_eq!(sum_of_multiplications, 48);
    }
